...
- 新增「星星速度」设置，仅调整 Slide 路径的提前显现时间；`0` 对应 4.4.0 默认效果，不改变移动、判定或 DJAuto 逻辑。
- 优化 D 区 Slide 与 Touch Slide 的等距路径绘制、星星朝向和终点判定特效位置。
- `COLOR` 染色覆盖 Tap、Each、Break、Hold、Star 与 Slide，并保留原素材纹理、高光与暗边。
- 优化媒体时间线的中途播放、暂停续播、同轨重叠恢复及 PV / BGA 等比缩放。
...
- 开启判定特效时，判定特效部分会错位，这是修sv负数特效bug的副产物。
