...
- `SV`、`HS`、`SPAWN`、`SIZE` 等命令支持按音符类型单独设置；全局 `SIZE` 不再缩放 Slide 轨迹，需使用 `slide=` 显式调整，SV可以控制Slide移动的速度。
- 新增 Touch Slide，支持普通键与 A/B/C/D/E Touch 区互连、连续路径、无头以及 Break 头 / Break 轨迹，例如 `1d-E5[8:1]`、`A1<A3<A5[8:1]`、`A1b-A2b[8:1]`，目前只支持圆弧和直线星星。
